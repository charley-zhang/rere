...
class DatasetHandler(VisionDatasetHandler):
    ...
    def __init__(self, descriptor, name=''):
        ...
        if isinstance(descriptor, str):
            assert os.path.isdir(descriptor)
    ...
    def _get_dataframe(self, ds_dir):
        ...
        for subset_id, subset_dirname in enumerate(subset_dirs): 
            ...
            for classid_order, class_dirname in enumerate(class_dirnames): 
                ...
                for filename in files: # loop over files in class
                    ...
                    df_dict['size'].append(images.get_dimensions(fullpath))
    ...
    def update(self, newdf=None):
        ...
        classids = sorted(self.df.classid.unique())
        self.classnames = [None]*len(classids)
        
        for _, r in self.df.iterrows():
            ci, cn = r['label']
            newci = classids.index(ci)
            r['label'] = (newci, cn)
            self.classnames[newci] = cn
